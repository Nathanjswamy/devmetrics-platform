...
export default function IntelligencePage() {
  ...
  if (isLoading || !aiInsights) {
    ...
    /* ── derived ── */
    const criticalCount = aiInsights.filter((i) => i.priority === "critical").length;
    const warningCount = aiInsights.filter((i) => i.priority === "high").length;
    const positiveCount = aiInsights.filter((i) => i.priority === "low").length;
    const infoCount = aiInsights.filter((i) => i.priority === "info").length;
      ...
      <main className="flex-1 p-8 max-w-7xl mx-auto w-full space-y-8">
        ...
        <div className="flex items-center gap-2">
          {[
            ...
            { key: "high", label: `Warnings (${warningCount})` },
            { key: "low", label: `Positive (${positiveCount})` },

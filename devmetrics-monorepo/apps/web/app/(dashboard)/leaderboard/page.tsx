...
function AvatarBadge({ avatar, rank }: { avatar?: string; rank: number }) {
  const safeInitials = avatar && !avatar.startsWith('http') ? avatar : "?";
  const colors = [
    "#D05A44", // Terracotta
    "#2B6B6D", // Teal
    "#787B4E", // Olive
  ...
  return (
    <div className="relative">
      <div
        className="w-10 h-10 rounded-sm flex items-center justify-center text-sm font-bold text-[#F8F6F1] overflow-hidden"
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <main className="flex-1 p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          {sorted.slice(0, 3).map((dev, i) => {
            ...
            return (
              <div
                key={dev.id}
                className="bauhaus-card p-5 transition-transform hover:-translate-y-0.5 flex flex-col"
              ...
              >
                ...
                <div className="flex items-center gap-3 mb-4">
                  <AvatarBadge avatar={dev.avatar} rank={rank} />
                  <div className="min-w-0 flex-1">
                    <div className="font-semibold text-text-primary text-sm truncate">{dev.name}</div>
                    <div className="text-xs text-text-muted truncate">{dev.role}</div>
                  </div>
        ...
        {/* Full table */}
        <div className="bauhaus-card overflow-x-auto">
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center gap-2.5 min-w-0 pr-4">
                  <div
                    className="w-8 h-8 rounded-sm flex items-center justify-center text-[10px] font-bold text-text-primary shrink-0 overflow-hidden"
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="text-sm font-semibold text-text-primary group-hover:text-indigo-300 transition-colors truncate">
                      {dev.name}
                    </div>
                    <div className="text-[10px] text-text-muted truncate">{dev.role}</div>
                  </div>
                </div>
